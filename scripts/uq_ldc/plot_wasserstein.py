...
# wasserstein metrics for lid-driven cavity on quadrilateral meshes
def wasserstein_quadMeshes(dir_sol, dir_fig, metric, show_plot, save_plot):
    
    fn = dir_sol+'wasserstein_'+str(metric)+'pt.json'
    plotOutfile = dir_fig+'wasserstein_ldc_'+str(metric)+'pt.pdf'
    # plotTitle = r'{Wasserstein distance between $N\times N$ and $2N\times 2N$, '+str(metric)+'-pt. dist.}'
    plotTitle = r'Wasserstein '+str(metric)+r'-pt. btw $N_{\mathbf{x}}\!\times\!N_{\mathbf{x}}$ and $2N_{\mathbf{x}}\!\times\! 2N_{\mathbf{x}}$'
    ...
    ref_wdVy = np.exp(np.polyval(linfit_wdVy, np.log(Nx))-0.2)
    ...
    myPlotDict['ref_data_labels'] = ['$O(N_{\mathbf{x}}^{%4.2f})$'%slope_wdV, '$O(N_{\mathbf{x}}^{%4.2f})$'%slope_wdVec]
