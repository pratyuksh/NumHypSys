...
//! Constructors
IncompNSFEM
:: IncompNSFEM (const nlohmann::json& config)
    : m_config (config)
{
    m_deg = config["deg_x"];
    m_nfluxType = "upwind";
...
void IncompNSFEM :: set(std::shared_ptr<Mesh>& mesh)
{
...
void IncompNSFEM :: init(const double dt)
{
...
:: init_assemble_system(const double dt)
{
    ...
    {
        assemble_diffusion();
        auto tmp = new SparseMatrix(*m_diffusion);
        (*tmp) *= dt*m_viscosity;
        m_massDiffusion = Add(*m_mass, *tmp);
        delete tmp;
    }
...
void IncompNSFEM :: assemble_mass()
{
    BilinearForm *mass_form
            = new BilinearForm(m_fespaces[0]);
...
void IncompNSFEM :: assemble_divergence()
{
    ...
    m_divNoBCs = div_form->LoseMat();
    delete div_form;

    //m_divT = Transpose(*m_div);
    m_divT = Transpose(*m_divNoBCs);
    eliminate_rows(*m_divT);
    m_div = Transpose(*m_divT);
}

//! Assembles SIP diffusion operator
void IncompNSFEM :: assemble_diffusion()
{
...
void IncompNSFEM
:: update_convection(GridFunction *v)
{
    reset_convection();
    ...
    mymfem::MyBilinearForm *convPlusNFlux_form
            = new mymfem::MyBilinearForm(m_fespaces[0]);
    convPlusNFlux_form->MyAddDomainIntegrator(m_convInt);
    convPlusNFlux_form->MyAddInteriorFaceIntegrator
            (m_numFluxInt);
    convPlusNFlux_form->MyAssemble(v);
    convPlusNFlux_form->Finalize();
    m_convNflux = convPlusNFlux_form->LoseMat();
    delete convPlusNFlux_form;

    /*auto stop = high_resolution_clock::now();
    auto duration = duration_cast<milliseconds>
            (stop - start);
    std::cout << "\n\nConvection assembly time: "
              << duration.count() << std::endl;*/
}

void IncompNSFEM
:: update_convection(Vector& V)
{
    auto v = std::make_unique<GridFunction>();
    v->MakeRef(m_fespaces[0], V);
    update_convection(v.get());
}

//! Assembles open boundary operator
//! Open boundary form:
//! \int_{\partial \Omega} (v_h \cdot n) p_h dS
void IncompNSFEM :: assemble_openBdry()
{
...
void IncompNSFEM :: assemble_invMass()
{
    auto invMassBf
            = new BilinearForm(m_fespaces[0]);
    ...
    invMassBf->Assemble();
    invMassBf->Finalize();
    m_invMass = invMassBf->LoseMat();
}

//! Assembles source
void IncompNSFEM :: assemble_source(const double t,
                                    Vector& b)
{
...
//! Assembles boundary
void IncompNSFEM :: assemble_bdry(const double t,
                                  Vector& b)
{
...
void IncompNSFEM :: update_vBdry(double t)
{
    m_vBdryCoeff->SetTime(t);
    (*m_vBdry) = 0.0;
...
void IncompNSFEM :: apply_BCs(Vector& z) const
{
    z.SetSubVector(m_ess_tdof_list, 0.0);
}

//! Applies boundary conditions to BlockVector
void IncompNSFEM :: apply_BCs(BlockVector &B) const
{
...
void IncompNSFEM :: apply_vBdry(Vector &B0) const
{
    for (int i=0; i<m_ess_tdof_list.Size(); i++) {
        B0(m_ess_tdof_list[i])
                = m_vBdry->Elem(m_ess_tdof_list[i]);
    }
}

//! Applies boundary conditions to SparseMatrix
void IncompNSFEM :: apply_BCs(SparseMatrix& A) const
{
